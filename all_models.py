...
def save_predictions(test_df, model_name, predictions):
    """Saves test predictions to CSV file"""
    submission = pd.DataFrame({"Id": test_df.index, "SalePrice": np.expm1(predictions)}) 
    filename = f"predicted_results/{model_name}.csv"
    submission.to_csv(filename, index=False)
    print(f"Predictions saved to {filename}!")

def plot_results(y_valid, preds, model_name):
    ...
    plt.title(f"{model_name.upper()} - Actual vs Predicted Prices")
    plt.legend()
    plt.savefig(f"graphs/{model_name}_plot.png")
    plt.close()
...
def main():
    ...
    for model_name, model in models.items():
        rmse, r2 = evaluate_model(model, X_valid, y_valid)
        results.append((model_name.upper(), rmse, r2))
    ...
    for model_name, model in models.items():
        ...
        save_predictions(test_df, model_name, test_preds)

        # Plot results
        plot_results(y_valid, model.predict(X_valid), model_name)
